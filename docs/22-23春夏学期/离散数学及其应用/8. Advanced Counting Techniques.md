...
Ordinary generating function of $\{a_n\}$: $G(x)=\sum_{k=0}^{\infin}a_kx^k$ (**0-index**)
...
>Eg. $a_n=C(10,n+1)$. $\sum_{n=0}^\infin C(10,n+1)x^n=\frac{1}{x}\sum_{n=1}^\infin C(10,n)x^n=\frac{(1+x)^{10}-1}{x}$
...
$$
(1+x)^a=\sum_{k=0}^{\infin} \frac{a(a-1)\dots (a-k+1)}{k!}x^k=\sum_{k=0}^{\infin}\binom{a}{k}x^k(a \in \mathbb{R})
$$
展开$(1+x)^{-n}$即可
...
(etc. 3=1+2=2+1)  $\sum_{k=0}^{\infin}(x+x^2+x^5)=\frac{1}{1-x-x^2-x^5}$
...
$\lim_{n \to \infin}\frac{D_n}{n!}=\frac{1}{e}$
$$
